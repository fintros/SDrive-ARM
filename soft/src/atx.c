...
static struct atxTrackInfo gTrackInfo[MAX_TRACK]; // pre-calculated info for each track
static u16 gBytesPerSector;                       // number of bytes per sector
static u16 gLastAngle;
static u08 gSectorsPerTrack; // number of sectors in each track
static u08 gCurrentHeadTrack;
...
u32 ReadUnalignedLE32(unsigned char *ptr)
{
    u32 result = 0;
    result |= *ptr++;
    result |= (*ptr++) << 8;
...
u16 ReadUnalignedLE16(unsigned char *ptr)
{
    u32 result = 0;
    result |= *ptr++;
    result |= (*ptr++) << 8;
...
u16 loadAtxSector(HWContext *ctx, file_t *pDisk, unsigned char *buffer, u16 num, unsigned short *sectorSize)
{
    ...
    *sectorSize = gBytesPerSector;
    ...
    if (trackHeader->trackNumber == tgtTrackNumber - 1 && sectorCount)
    {
            ...
            for (i = 0; i < sectorCount; i++)
            {
                if (faccess_offset(ctx, pDisk, FILE_ACCESS_READ, currentFileOffset, buffer, sizeof(struct atxSectorHeader)))
                {
                    ...
                    if (!(sectorHeader->status & MASK_FDC_MISSING) && sectorHeader->number == tgtSectorNumber)
                    {
                        ...
                        int tt = sectorHeader->timev - req_head_pos;
        ...
        waitForAngularPosition(incAngularDisplacement(gLastAngle, au_one_sector_read), 0);
