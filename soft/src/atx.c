...
u16 loadAtxSector(u16 num, unsigned short *sectorSize, u08 *status) {
    ...
    u08 hasError = FALSE;   // flag for drive status errors
    ...
    *sectorSize = gBytesPerSector;
    ...
    if (is_1050()) {
        CyDelayUs(MS_DRIVE_REQUEST_DELAY_1050*1000);
    } else {
        CyDelayUs(MS_DRIVE_REQUEST_DELAY_810*1000);
    }
    ...
    if (gCurrentHeadTrack != tgtTrackNumber) {
        ...
        for (i = 0; i < diff; i++) {
            if (is_1050()) {
                CyDelayUs(MS_TRACK_STEP_1050*1000);
            } else {
                CyDelayUs(MS_TRACK_STEP_810*1000);
            }
        }
        // delay for head settling
        if (is_1050()) {
            CyDelayUs(MS_HEAD_SETTLE_1050*1000);
        } else {
            CyDelayUs(MS_HEAD_SETTLE_810*1000);
        }
    }
	...
	currentFileOffset += slHeader->next - sectorCount * sizeof(struct atxSectorHeader);
        ...
        if (is_1050()) {
            CyDelay(MS_CRC_CALCULATION_1050);
        } else {
            CyDelay(MS_CRC_CALCULATION_810);
        }
    }
