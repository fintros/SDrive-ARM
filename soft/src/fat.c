...
unsigned char fatGetDirEntry(HWContext *ctx, file_t *pDisk, unsigned char *file_name, unsigned short entry, unsigned char use_long_names)
{
	...
	if (!f->last_dir_valid || (entry <= f->last_dir_entry && (entry != f->last_dir_entry || f->last_dir_valid != 1 || use_long_names != 0)))
	{
	...
	while (1)
	{
		...
		else
		{
			...
			if (de->deAttributes == ATTR_LONG_FILENAME)
			{
				...
				for (i = 0; i < 5; i++)
					*fnbPtr++ = we->wePart1[i * 2]; // copy first part
				for (i = 0; i < 6; i++)
					*fnbPtr++ = we->wePart2[i * 2]; // second part
				for (i = 0; i < 2; i++)
					*fnbPtr++ = we->wePart3[i * 2]; // and third part

				if (we->weCnt & WIN_LAST)
					*fnbPtr = 0; // in case dirnamelength is multiple of 13, add termination
				if ((we->weCnt & 0x0f) == 1)
					haveLongNameEntry = 1; // flag that we have a complete long name entry set
			}
			else
			{
